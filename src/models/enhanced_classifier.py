...
class GeminiEnhancedClassifier:
    ...
    def __init__(self, api_key: Optional[str] = None, traditional_model_path: str = "models/telco_ticket_classifier.pkl"):
        ...
        self.traditional_classifier = TicketClassificationPipeline() if TicketClassificationPipeline else None
        self.has_traditional_models = False  # Initialize flag
    ...
    def _load_traditional_model(self, model_path: str):
        ...
        try:
            ...
            # Only proceed if we have a valid traditional classifier
            if self.traditional_classifier:
                self.traditional_classifier.models = model_data.get('models', {})
                self.traditional_classifier.ensemble_weights = model_data.get('ensemble_weights', {})
                # Handle training_history type mismatch
                training_hist = model_data.get('training_history', {})
                if isinstance(training_hist, list):
                    self.traditional_classifier.training_history = {}
                else:
                    self.traditional_classifier.training_history = training_hist
                
                # Check if models are actually trained
                self.has_traditional_models = len(self.traditional_classifier.models) > 0 and 'logistic_regression' in self.traditional_classifier.models
                
                if self.has_traditional_models:
                    logger.info(f"✅ Traditional model loaded from {model_path}")
                else:
                    logger.warning(f"⚠️ Traditional model file exists but no trained models found - using LLM-only mode")
            else:
                logger.warning(f"⚠️ Traditional classifier not available - using LLM-only mode")
                self.has_traditional_models = False
                
        except Exception as e:
            logger.warning(f"⚠️ Could not load traditional model: {e} - using LLM-only mode")
            self.has_traditional_models = False
...
"""
    ...
    def classify_ticket(self, ticket_text: str) -> EnhancedClassificationResult:
        ...
        # Get traditional model prediction (if available)
        if self.has_traditional_models and self.traditional_classifier:
            try:
                ...
                traditional_prob_dict = dict(zip(base_categories, traditional_proba, strict=True))
                traditional_confidence = max(traditional_proba)
            except Exception as e:
                logger.warning(f"⚠️ Traditional model prediction failed: {e} - using LLM-only mode")
                self.has_traditional_models = False
                traditional_pred = "TECHNICAL"  # Default fallback
                traditional_confidence = 0.1   # Low confidence
                traditional_prob_dict = {}
        else:
            # LLM-only mode
            traditional_pred = "TECHNICAL"  # Default fallback
            traditional_confidence = 0.1   # Low confidence  
