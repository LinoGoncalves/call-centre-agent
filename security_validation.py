...
class SecurityValidator:
    ...
    def test_network_binding(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                details = (
                    f"Insecure bindings found: {insecure_bindings}"
                    if not passed
                    else "All services bound to localhost"
                )
                self.log_result(
                    "Network Localhost Binding", passed, "CRITICAL", details
                )
            else:
                self.log_result(
    ...
    def test_jupyter_authentication(self):
        ...
        try:
            ...
            if (
                ...
                or "token" in response.text.lower()
            ):
                passed = True
            ...
            self.log_result(
                "Jupyter Authentication Required", passed, "CRITICAL", details
            )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                details = (
                    f"Issues: {issues}"
                    if issues
                    else "Container capabilities properly restricted"
                )
                self.log_result(
                    "Container Capability Restriction", passed, "HIGH", details
                )
            else:
                self.log_result(
    ...
    def test_file_permissions(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                details = (
                    f"World-readable files: {world_readable}"
                    if not passed
                    else "File permissions properly restricted"
                )
                self.log_result("File Permissions Security", passed, "MEDIUM", details)
            else:
                self.log_result(
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                details = (
                    ...
                    if not passed
                    else "Environment variables secure"
                )
                self.log_result(
                    "Environment Variables Security", passed, "MEDIUM", details
                )
            else:
                self.log_result(
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                details = (
                    f"Issues: {issues}"
                    if issues
                    else f"Resource limits: Memory={memory / 1024 / 1024 / 1024:.1f}GB, CPUs={cpus / 1000000000:.1f}"
                )
                self.log_result("Resource Limits Enforced", passed, "MEDIUM", details)
            else:
                self.log_result(
    ...
    def generate_report(self):
        ...
        if self.critical_failures == 0 and self.high_failures == 0:
            print(
                "✅ PRODUCTION READY - All critical and high-priority security tests passed"
            )
